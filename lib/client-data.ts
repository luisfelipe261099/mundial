...
export async function getOrdem(id: string, clientId: string): Promise<OrdemServico | null> {
  const o = await prisma.serviceOrder.findFirst({ where: { id, clientId }, include: { items: { orderBy: { id: "asc" } } } });
...
export async function getOrcamento(id: string, clientId: string): Promise<Orcamento | null> {
  const b = await prisma.budget.findFirst({ where: { id, clientId }, include: { items: { orderBy: { id: "asc" } } } });
